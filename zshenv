...
# ensure that we load the nix-daemon.sh configuration file when it exists
#
# the nix installer automatically appends these lines to /etc/zshrc, but
# when macOS is upgraded, /etc/zshrc is overwritten and the lines are lost
#
# this is technically platform-specific, but we need it in zshenv so it is
# sourced regardless of the interactivity of the shell
#
...
# Add nix profile binaries, if ~/.nix-profile link exists
...
# git worktrees
#---------------
# TODO(jordan): move all this stuff into ~/.local/zsh-functions ?
